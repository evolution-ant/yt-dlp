...
from .stories import STORIESIE
from .common import InfoExtractor
from .youtube import YoutubeIE
from ..compat import compat_urlparse


class AudibleVideoIE(STORIESIE):
    IE_NAME = 'audible:video'
    _VALID_URL = r'''(?x)
                    https?://
                        (?:
                            stories\.audible\.com/.*
    ...
    _TESTS = [{
        'url': 'https://stories.audible.com/pdp/B015D78L0U?ref=adbl_ent_anon_ds_pdp_pc_cntr-1-2',
        'info_dict': {
            'id': 'B015D78L0U',
            'ext': 'mp4',
            'display_id': 'B015D78L0U',
    ...
    def _real_extract(self, url):
        id = re.findall(".*/pdp/(.*)\?ref.*",url)[0]
        id = id.replace( '/' , '' )  # bugfix, id may end with / and \
        id = id.replace( '\\' , '' )
        info_dict = self._extract_feed_info(id)
        return info_dict
