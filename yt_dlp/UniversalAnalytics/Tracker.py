...
from ..utils import YoutubeDLHTTPSHandler, YoutubeDLHandler
import datetime
import time
import uuid
import hashlib
...
class HTTPRequest(object):
    ...
    def __init__(self, user_agent = None, *args, **opts):
        ...
        opener = compat_urllib_request.build_opener(YoutubeDLHTTPSHandler(params), YoutubeDLHandler(params))
    ...
    def send(self, data):
        request = compat_urllib_request.Request(
                self.endpoint + '?' + compat_urllib_parse_urlencode(self.fixUTF8(data)),
                headers = {
    ...
    def open(self, request):
        try:
            return compat_urllib_request.urlopen(request, timeout=2)
        except:
            return False
...
class HTTPPost(HTTPRequest):
    ...
    def send(self, data):
        request = compat_urllib_request.Request(
                self.endpoint,
                data = compat_urllib_parse_urlencode(self.fixUTF8(data)),
                headers = {
                    'User-Agent': self.user_agent
                }
            )
