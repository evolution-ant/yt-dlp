    ...
    def _start_frag_download(self, ctx):
        ...
        def frag_progress_hook(s):
            ...
            if not ctx['live']:
                ...
                if self.params.get('fab_t', False):
                    tmpfilename = ctx['tmpfilename']
                    write_string(json.dumps({
                        'tmp_files': [tmpfilename, encodeFilename(self.ytdl_filename(ctx['filename'])), '%s-Frag%d' % (ctx['tmpfilename'], ctx['fragment_index'])+'.part'],
                        'estimated_size':
                        estimated_size
                    }),
                                out=None,
