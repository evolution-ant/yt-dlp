...
class FFmpegPostProcessor(PostProcessor):
    ...
    def _determine_executables(self):
        ...
        def get_ffmpeg_version(path):
            ...
            ver = "testversion" if self._downloader.params.get('ffmpegargs', False) and path is "ffmpeg" else get_exe_version(path, args=['-version'])
            if ver:
                regexs = [
    ...
    @property
    def available(self):
        return self.basename is not None or self._downloader.params.get('ffmpegargs', False)
    ...
    def get_audio_codec(self, path):
        ...
        try:
            ...
            if self._downloader.params.get('ffmpegargs', False):
                 self._downloader.to_screen(
                    '[ffmpegargs] %s' % cmd)
                 output = ""
                 while 1:
    ...
    def get_ffmpeg_multiple_args(self, input_paths, out_path, opts):
        oldest_mtime = min(
        ...
        self._downloader.to_screen("[ffmpegargs] %s" % cmd)
        self._downloader.params['keepvideo'] = True

    def get_ffmpeg_multiple_files(self, input_paths, out_path, opts):
        self.check_version()
    ...
    def run_ffmpeg_multiple_files(self, input_paths, out_path, opts):
        ...
        cmd += (files_cmd
                ...
                + [encodeFilename(self._ffmpeg_filename_argument(out_path), True)])
        # zjl elk report
        set_ffmpeg_start_time(datetime.datetime.now())
        # end
        if self._downloader.params.get('verbose', False):
    ...
    def run_ffmpeg(self, path, out_path, opts):
        if self._downloader.params.get("ffmpegargs", False):
            self.get_ffmpeg_multiple_args([path], out_path, opts)
        else:
...
class FFmpegEmbedSubtitlePP(FFmpegPostProcessor):
    def run(self, information):
        ...
        if not self._downloader.params.get("ffmpegargs", False):
...
class FFmpegMetadataPP(FFmpegPostProcessor):
    def run(self, info):
        ...
        if  self._downloader.params.get("ffmpegargs", False):
            self.get_ffmpeg_multiple_files(in_filenames, temp_filename, options)
        else:
...
class FFmpegMergerPP(FFmpegPostProcessor):
    def run(self, info):
        ...
        if self._downloader.params.get('ffmpegargs', False):
            self.get_ffmpeg_multiple_args(info['__files_to_merge'], encodeFilename(filename), args)
        else:
...
class FFmpegFixupM4aPP(FFmpegPostProcessor):
    def run(self, info):
        if info.get('container') != 'm4a_dash' or self._downloader.params.get("ffmpegargs", False):
            return [], info
